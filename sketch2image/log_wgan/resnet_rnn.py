...
def lstm_conv_block(inp, ht, ct, filter_depth, stride,
    ...
    full_inp_p = tf.concat([inp, ht, ct], axis=channel_index)    # peephole connection
    if stride == 2:
        # input gate
        ig = conv_mean_pool(full_inp_p, filter_depth, 3, rate=1, data_format=data_format,
                            normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                            activation_fn=tf.nn.sigmoid,
                            weights_regularizer=regularizer,
                            weights_initializer=weights_initializer)
        # forget gate
        fg = conv_mean_pool(full_inp_p, filter_depth, 3, rate=1, data_format=data_format,
                            normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                            activation_fn=tf.nn.sigmoid,
                            weights_regularizer=regularizer,
                            weights_initializer=weights_initializer)
        # output gate
        og = conv_mean_pool(full_inp_p, filter_depth, 3, rate=1, data_format=data_format,
                            normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                            activation_fn=tf.nn.sigmoid,
                            weights_regularizer=regularizer,
                            weights_initializer=weights_initializer)
        ...
        g = conv_mean_pool(full_inp, filter_depth, 3, rate=1, data_format=data_format,
                           ...
                           weights_regularizer=regularizer,
                           weights_initializer=weights_initializer)
        g = ly.conv2d(g, filter_depth, 3, rate=1, data_format=data_format,
        ...
        with tf.variable_scope('lstm_conv_cell_state_normalization'):
            if normalizer_fn is not None:
                normalizer_params = normalizer_params or {}
                ct_new = normalizer_fn(ct_new, activation_fn=None, **normalizer_params)

        # new hidden state
        ht_new = activation_fn(ct_new) * og + bias_h
    elif stride == 1:
        # input gate
        ig = ly.conv2d(full_inp_p, filter_depth, 3, stride=stride, rate=dilate, data_format=data_format,
                       normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                       activation_fn=tf.nn.sigmoid,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
        # forget gate
        fg = ly.conv2d(full_inp_p, filter_depth, 3, stride=stride, rate=dilate, data_format=data_format,
                       normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                       activation_fn=tf.nn.sigmoid,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
        # output gate
        og = ly.conv2d(full_inp_p, filter_depth, 3, stride=stride, rate=dilate, data_format=data_format,
                       normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                       activation_fn=tf.nn.sigmoid,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
        ...
        g = ly.conv2d(full_inp, filter_depth, 3, stride=stride, rate=dilate, data_format=data_format,
                      ...
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
        g = ly.conv2d(g, filter_depth, 3, stride=stride, rate=dilate, data_format=data_format,
                      normalizer_fn=None, activation_fn=None,
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
...
def gru_conv_block(inp, ht, filter_depth, stride,
                   dilate=1, first_layer_no_dilate=None,
                   norm_activate_first=None,
                   activation_fn=tf.nn.relu,
                   normalizer_fn=None,
    ...
    # update gate
    rg = ly.conv2d(full_inp, ht.get_shape().as_list()[channel_index], 3, stride=1, rate=dilate,
                   data_format=data_format, activation_fn=tf.nn.sigmoid,
                   normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                   weights_regularizer=regularizer,
                   weights_initializer=weights_initializer)
    # output gate
    zg = ly.conv2d(full_inp, filter_depth, 3, stride=1, rate=dilate, data_format=data_format,
                   normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                   activation_fn=tf.nn.sigmoid,
                   weights_regularizer=regularizer,
                   weights_initializer=weights_initializer)
    ...
    # new hidden state
    h_new = ly.conv2d(tf.concat([rg * ht, inp], axis=channel_index), filter_depth, 3, stride=1, rate=dilate,
                      data_format=data_format, activation_fn=activation_fn,
                      normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
    h_new = ly.conv2d(h_new, filter_depth, 3, stride=1, rate=dilate,
                      data_format=data_format, activation_fn=activation_fn,
                      normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
    ...
    # bias variables
    bias_shape = h_new.get_shape().as_list()
    bias_shape[0] = 1
    bias_h = tf.get_variable("hidden_out_bias", shape=bias_shape, dtype=tf.float32, initializer=tf.zeros_initializer())

    # new hidden state out
    # linear project for filter depth
    if ht.get_shape().as_list()[channel_index] != filter_depth:
        ht = ly.conv2d(ht, filter_depth, 1, stride=1, data_format=data_format, activation_fn=activation_fn,
                       normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
    ht_new = ht * (1 - zg) + h_new * zg + bias_h
...
def lstm_deconv_block(inp, ht, ct, filter_depth, stride,
    ...
    full_inp = tf.concat([inp, ht], axis=channel_index)
    full_inp_p = tf.concat([inp, ht, ct], axis=channel_index)        # peephole connections
    if stride == 2:
        # input gate
        ig = upsample_conv(full_inp_p, filter_depth, 3, data_format=data_format,
                           normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                           activation_fn=tf.nn.sigmoid,
                           weights_regularizer=regularizer,
                           weights_initializer=weights_initializer)
        # forget gate
        fg = upsample_conv(full_inp_p, filter_depth, 3, data_format=data_format,
                           normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                           activation_fn=tf.nn.sigmoid,
                           weights_regularizer=regularizer,
                           weights_initializer=weights_initializer)
        # output gate
        og = upsample_conv(full_inp_p, filter_depth, 3, data_format=data_format,
                           normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                           activation_fn=tf.nn.sigmoid,
                           weights_regularizer=regularizer,
                           weights_initializer=weights_initializer)
        ...
        g = upsample_conv(full_inp, filter_depth, 3, data_format=data_format,
                          ...
                          weights_regularizer=regularizer,
                          weights_initializer=weights_initializer)
        g = ly.conv2d(g, filter_depth, 3, stride=1, data_format=data_format,
                      normalizer_fn=None, activation_fn=None,
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
        ...
        ct_new = upsample_conv(ct, filter_depth, 1, data_format=data_format,
                               ...
                               weights_regularizer=regularizer, biases_initializer=None,
                               weights_initializer=weights_initializer) * fg + g * ig + bias_c
        with tf.variable_scope('lstm_deconv_cell_state_normalization'):
            if normalizer_fn is not None:
                normalizer_params = normalizer_params or {}
                ct_new = normalizer_fn(ct_new, activation_fn=None, **normalizer_params)
        ...
        # input gate
        ig = ly.conv2d(full_inp_p, filter_depth, 3, stride=stride, data_format=data_format,
                       normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                       activation_fn=tf.nn.sigmoid,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
        # forget gate
        fg = ly.conv2d(full_inp_p, filter_depth, 3, stride=stride, data_format=data_format,
                       normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                       activation_fn=tf.nn.sigmoid,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
        # output gate
        og = ly.conv2d(full_inp_p, filter_depth, 3, stride=stride, data_format=data_format,
                       normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                       activation_fn=tf.nn.sigmoid,
                       weights_regularizer=regularizer,
                       weights_initializer=weights_initializer)
        ...
        #                    weights_initializer=weights_initializer) * fg + g * ig
        # # new hidden state
        # ht_new = ly.conv2d(ct_new, filter_depth, 3, stride=stride, data_format=data_format,
        #                    normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
        #                    activation_fn=activation_fn,
        #                    weights_regularizer=regularizer,
        #                    weights_initializer=weights_initializer) * og
...
def gru_deconv_block(inp, ht, filter_depth, stride,
                     norm_activate_first=None,
                     activation_fn=tf.nn.relu,
                     normalizer_fn=None,
                     normalizer_params=None,
    ...
    if stride == 2:
        inp = upsample(inp, data_format=data_format)
    ...
    # update gate
    rg = ly.conv2d(full_inp, ht.get_shape().as_list()[channel_index], 3, stride=1, data_format=data_format,
                   normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                   activation_fn=tf.nn.sigmoid,
                   weights_regularizer=regularizer,
                   weights_initializer=weights_initializer)
    # output gate
    zg = ly.conv2d(full_inp, filter_depth, 3, stride=1, data_format=data_format,
                   normalizer_fn=mask_normalizer_fn, normalizer_params=mask_normalizer_params,
                   activation_fn=tf.nn.sigmoid,
                   weights_regularizer=regularizer,
                   weights_initializer=weights_initializer)
    ...
    # new hidden state
    h_new = ly.conv2d(tf.concat([rg * ht, inp], axis=channel_index), filter_depth, 3, stride=1, data_format=data_format,
                      normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
                      activation_fn=activation_fn,
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
    h_new = ly.conv2d(h_new, filter_depth, 3, stride=1, data_format=data_format,
                      normalizer_fn=normalizer_fn, normalizer_params=normalizer_params,
                      activation_fn=activation_fn,
                      weights_regularizer=regularizer,
                      weights_initializer=weights_initializer)
    ...
    # linear project for filter depth
    if ht.get_shape().as_list()[channel_index] != filter_depth:
        ht = ly.conv2d(ht, filter_depth, 1, stride=1, data_format=data_format, activation_fn=None,
...
def unrolled_lstm_conv(x, ht, ct, filter_depth, stride=2, dilate_rate=1,
    ...
    cell_block = lstm_conv_block

    hts_new = []
    cts_new = []
    inp = x
    with tf.variable_scope('lstm_conv_unit_t_%d_layer_0' % unit_num):
        ht_new, ct_new = cell_block(inp, ht[0], ct[0], filter_depth, stride=stride,
    ...
    for i in range(1, num_blocks):
        with tf.variable_scope('lstm_deconv_unit_t_%d_layer_%d' % (unit_num, i)):
            ht_new, ct_new = cell_block(inp, ht[i], ct[i], filter_depth, stride=1,
                                        dilate=dilate_rate, first_layer_no_dilate=None,
                                        norm_activate_first=None,
                                        activation_fn=activation_fn,
                                        normalizer_fn=normalizer_fn,
...
def unrolled_lstm_deconv(x, ht, ct, filter_depth, stride=2, num_blocks=2,
    ...
    cell_block = lstm_deconv_block

    hts_new = []
    cts_new = []
    inp = x
    with tf.variable_scope('lstm_deconv_unit_t_%d_layer_0' % unit_num):
        ht_new, ct_new = cell_block(inp, ht[0], ct[0], filter_depth, stride=stride, norm_activate_first=None,
                                    activation_fn=activation_fn,
                                    normalizer_fn=normalizer_fn,
                                    normalizer_params=normalizer_params,
                                    weights_initializer=weights_initializer,
    ...
    for i in range(1, num_blocks):
        with tf.variable_scope('lstm_deconv_unit_t_%d_layer_%d' % (unit_num, i)):
            ht_new, ct_new = cell_block(inp, ht[i], ct[i], filter_depth, stride=1, norm_activate_first=None,
                                        activation_fn=activation_fn,
                                        normalizer_fn=normalizer_fn,
                                        normalizer_params=normalizer_params,
                                        weights_initializer=weights_initializer,
...
def unrolled_gru_conv(x, ht, filter_depth, stride=2, dilate_rate=1,
                      num_blocks=5,
                      first_unit=False, last_unit=False,
                      activation_fn=tf.nn.relu,
                      normalizer_fn=None,
    ...
    cell_block = gru_conv_block

    hts_new = []
    inp = x
    with tf.variable_scope('gru_conv_unit_t_%d_layer_0' % unit_num):
        ht_new = cell_block(inp, ht[0], filter_depth, stride=stride,
                            dilate=dilate_rate, first_layer_no_dilate=None,
                            norm_activate_first=None,
                            activation_fn=activation_fn,
                            normalizer_fn=normalizer_fn,
    ...
    for i in range(1, num_blocks):
        ...
        with tf.variable_scope('gru_deconv_unit_t_%d_layer_%d' % (unit_num, i)):
            ht_new = cell_block(inp, ht[i], filter_depth, stride=1,
                                dilate=dilate_rate, first_layer_no_dilate=None,
                                norm_activate_first=None,
                                activation_fn=activation_fn,
                                normalizer_fn=normalizer_fn,
...
def unrolled_gru_deconv(x, ht, filter_depth, stride=2, num_blocks=2,
                        first_unit=False, last_unit=False,
                        activation_fn=tf.nn.relu,
                        normalizer_fn=None,
                        normalizer_params=None,
    ...
    cell_block = gru_deconv_block

    hts_new = []
    inp = x
    with tf.variable_scope('gru_deconv_unit_t_%d_layer_0' % unit_num):
        ht_new = cell_block(inp, ht[0], filter_depth, stride=stride, norm_activate_first=None,
                            activation_fn=activation_fn,
                            normalizer_fn=normalizer_fn,
                            normalizer_params=normalizer_params,
                            weights_initializer=weights_initializer,
    ...
    for i in range(1, num_blocks):
        ...
        with tf.variable_scope('gru_deconv_unit_t_%d_layer_%d' % (unit_num, i)):
            ht_new = cell_block(inp, ht[i], filter_depth, stride=1, norm_activate_first=None,
                                activation_fn=activation_fn,
                                normalizer_fn=normalizer_fn,
                                normalizer_params=normalizer_params,
                                weights_initializer=weights_initializer,
